"""Shared logging setup for the flocroscope package.

Provides a single :func:`setup_logging` helper that configures the
root ``flocroscope`` logger with a consistent format, level, and
...
def setup_logging(
    ...
    """Configure logging for the flocroscope package.

    Sets up ``logging.basicConfig`` with a standard format and
    configures the ``flocroscope`` package logger to the
    ...
    pkg_logger = logging.getLogger("flocroscope")
