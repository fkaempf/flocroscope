...
def _reset_logging():
    """Reset the flocroscope logger state between tests."""
...
class TestSetupLogging:
    ...
    def test_sets_package_logger_level_info(self):
        """Default call sets flocroscope logger to INFO."""
    ...
    def test_file_handler_writes(self, tmp_path: Path):
        ...
        test_logger = logging.getLogger("flocroscope.test_write")
        test_logger.info("hello from test")

        # Flush handlers
        for h in logging.getLogger("flocroscope").handlers:
            h.flush()
